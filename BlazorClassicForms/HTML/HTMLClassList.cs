...
namespace BlazorClassicForms.HTML
{
	...
	public class HTMLClassList
	{
		...
		{
			return dom.JSRuntime.InvokeAsync<bool>("classList_contains", new object[]
			{
				this.owner.uid,
				value
			}).Result;
		...
		{
			dom.JSRuntime.InvokeAsync<bool>("classList_remove", new object[]
			{
				this.owner.uid,
				value
			});
		...
		{
			dom.JSRuntime.InvokeAsync<bool>("classList_add", new object[]
			{
				this.owner.uid,
				value
			});
