...
namespace BlazorClassicForms.HTML
{
	...
	public class HTMLWindow
	{
		...
		public HTMLWindow()
		{
			this.uid = dom.JSRuntime.Invoke<string>("_window", Array.Empty<object>());
		...
		{
			dom.JSRuntime.Invoke<object>("element_set", new object[]
			{
				this.uid,
				name,
				value
		...
		{
			return dom.JSRuntime.Invoke<long>("element_get", new object[]
			{
				this.uid,
				name
			});
		...
		public Action<Event> onresize
		{
			set
			{
				...
				dom.JSRuntime.Invoke<object>("onresize", new object[]
				{
					this.uid,
					text
				});
